...
cmd({
    ...
    filename: __filename
}, async (conn, mek, m, { from, q, reply, isPre , isMe, isOwner, isSudo}) => {
    if (isUploadinggg) {
...
//iwaraiiii
     ...
     // let searchTitle = cleanText.replace(/[^a-zA-Z0-9 ]/g, "").trim();
      let searchTitle = cleanText.replace(/\s+/g, " ");
        ...
        } else if (originalUrl.includes("pixeldrain.com")) {

    try {
        let formattedUrl = originalUrl.replace('/u/', '/api/file/');
        finalLink = formattedUrl;
    } catch (err) {
        finalLink = originalUrl;
    }
